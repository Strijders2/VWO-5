...
class Jos {
  ...
  wordtGeraakt(object) {
    // Checkt voor aanraking van hoogte en breedte van de vijand
    if (this.x + raster.celGrootte >= object.x &&
        this.x <= object.x + object.width &&
        this.y + raster.celGrootte >= object.y &&
        this.y <= object.y + object.height) {
      return true; // Aanraking gedetecteerd
    } else {
      return false;
    }
  }
...
class Vijand {
  constructor(x, y) {
    ...
    this.sprite = null;
    this.stapGrootte = null;
    this.width = raster.celGrootte; // Voegt breedte toe aan vijand
    this.height = raster.celGrootte; // Voegt hoogte toe aan vijand
  }
...
class Bom {
  constructor(x, y) {
    ...
    this.speed = random(0.2, 0.6); // Willekeruige snelheid voor elke bom, adjusted for slower movement
    
    // Initialize the bom's movement direction based on its initial position
    if (this.y === 0) {
      this.beweegRichting = 1; // naar beneden starten bewege
...
function draw() {
  ...
  // Checked als eve wordt geraakt door vijand
  if (eve.wordtGeraakt(alice) || eve.wordtGeraakt(bob)) {
    levens--; // Decrease the number of lives
  

    // Als leven eraf gaat, gaat eve terug naar orginele positie 
    if (levens > 0) {
      eve.x = 400; //Eve x positie wordt verandert
      eve.y = 300; // Eve y positie wordt verandert
    } else {
      noLoop(); // Stopt de game als er geen levens meer zijn
      background('red');
      fill('white');
      textSize(50);
